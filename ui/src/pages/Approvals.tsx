...
export function Approvals() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("nav.approvals") }]);
  }, [setBreadcrumbs, t]);
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("status.failed"));
    },
  });
  ...
  if (!selectedCompanyId) {
    return <p className="text-sm text-muted-foreground">{t("dashboard.selectCompany")}</p>;
  }
