...
const ACTION_LABELS: Record<string, string> = {
  "issue.created": "이슈를 생성했습니다",
  "issue.updated": "이슈를 수정했습니다",
  "issue.checked_out": "이슈를 체크아웃했습니다",
  "issue.released": "이슈를 릴리스했습니다",
  "issue.comment_added": "코멘트를 추가했습니다",
  "issue.attachment_added": "첨부 파일을 추가했습니다",
  "issue.attachment_removed": "첨부 파일을 제거했습니다",
  "issue.document_created": "문서를 생성했습니다",
  "issue.document_updated": "문서를 수정했습니다",
  "issue.document_deleted": "문서를 삭제했습니다",
  "issue.deleted": "이슈를 삭제했습니다",
  "agent.created": "에이전트를 생성했습니다",
  "agent.updated": "에이전트를 수정했습니다",
  "agent.paused": "에이전트를 일시정지했습니다",
  "agent.resumed": "에이전트를 재개했습니다",
  "agent.terminated": "에이전트를 종료했습니다",
  "heartbeat.invoked": "하트비트를 호출했습니다",
...
export function IssueDetail() {
  ...
  const sourceBreadcrumb = useMemo(
    () => readIssueDetailBreadcrumb(location.state) ?? { label: t("nav.issues"), href: "/issues" },
    [location.state],
  );
  ...
  const commentReassignOptions = useMemo(() => {
    ...
    if (currentUserId) {
      options.push({ id: `user:${currentUserId}`, label: t("issues.me") });
    }
    return options;
  ...
  const copyIssueToClipboard = async () => {
    ...
    setCopied(true);
    pushToast({ title: t("toast.copiedToClipboard"), tone: "success" });
    ...
    <>
      ...
      >
        ...
        {uploadAttachment.isPending || importMarkdownDocument.isPending ? "업로드 중..." : (
          <>
            <span className="hidden sm:inline">첨부 파일 업로드</span>
