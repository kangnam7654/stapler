...
export function NewAgent() {
  ...
  const createAgent = useMutation({
    ...
    onError: (error) => {
      setFormError(error instanceof Error ? error.message : t("agents.createAgent"));
    },
  });
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        <h1 className="text-lg font-semibold">{t("agents.newAgent")}</h1>
        <p className="text-sm text-muted-foreground mt-1">
          {t("agents.advancedAgentConfig")}
        </p>
      </div>
      ...
      <div className="border border-border">
        ...
        <div className="border-t border-border px-4 py-3">
          {isFirstAgent && (
            <p className="text-xs text-muted-foreground mb-2">{t("agents.willBeCeo")}</p>
          )}
          {formError && (
